...
from PagesObject.Pages.Cards.refundPage import RefundPage
...
class RefundActions:
    def __init__(self, driver, helps):
        ...
        self.errorsubmenu = self.submenu.clickSubmenu("Refund")
        if len(self.errorsubmenu) != 0:
            self.error.append(self.errorsubmenu)
        self.help.info_log(self.page, "Submenu Auth -> ok.")

        self.form = RefundPage(self.driver, self.help)

    def actionsClickCmbMerchant(self):

        errorClicCmbMerchant = self.form.clickCmbDataSeries()
