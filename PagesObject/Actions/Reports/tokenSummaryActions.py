...
from PagesObject.Pages.Reports.tokenSummaryPage import TokenSummaryPage


class TokenSummaryActions:
    def __init__(self, driver, helps):
        self.driver = driver
        self.help = helps
        self.error = list()
        self.page = "Token Summary Actions Page"
        ...
        self.errorsubmenu = self.submenu.clickSubmenu("TokenSummary")
        if len(self.errorsubmenu) != 0:
            self.error.append(self.errorsubmenu)
        self.help.info_log(self.page, "Submenu Auth -> ok.")

        self.form = TokenSummaryPage(self.driver, self.help)

    def actionsClickCmbDataSeries(self):

        errorClicCmbDataSeries = self.form.clickTextFilterBy()
