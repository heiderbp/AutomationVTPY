...
class MenuP:
    def __init__(self, driver, helps, flag=True):
        ...
        if flag:
            ...
            try:
                #app>div>nav>div.v-navigation-drawer__content>div>div:nth-child(2)>div>div.v-list-group__items>a:nth-child(1)
                self.menuAdministration = self.wait.until(ec.visibility_of_element_located((By.CSS_SELECTOR, "#app>div>nav>div.v-navigation-drawer__content>div>div:nth-child(2)>div"))) #Locators.menu.replace("MENU", "2"))))
            except Exception as e:
